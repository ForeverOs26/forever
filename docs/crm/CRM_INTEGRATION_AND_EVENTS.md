...
6. **`units_touched` is suppressed** until a canonical `unit_availability_history` exists. The Owner requirement is two-thirds met and said so. The two signals that remain carry a **second, independent prerequisite** — a stable `(project_id, unit_code)` identity, and a producer for each watched table — before any unit-linked follow-up is enabled (§8.5–§8.7).
...
Gating dependencies: `crm_enquiry_attribution` and `crm_decision_profile` are Phase 2, so until then the capture transaction writes `crm_enquiry` and nothing else. `crm_opportunity`, `crm_unit_hold` and `crm_reservation` are Phase 2/3, so the project-change sweep cannot precede Phase 2 — and its unit-linked half cannot precede the `units` identity prerequisite (§8.5), which is ingest-subsystem work owned outside this package.
...
| 3 | Unit enquiry (`?project=&unit=`) | inbound, sync | Unresolvable slug/unit ⇒ `focus_*` NULL, `project_slug_at_capture` still recorded. Never creates a project or unit. A NULL `focus_unit_id` is **outside the §8.3 watch set by construction**, so that interest receives no automated change follow-up (§8.6). |
...
| 7 | Price / status changes | inbound, **async** (§8) | Identifiers and timestamps only, never values. A missed sweep re-runs from the cursor. Gated on §8.7: `public.project_status_history` has no writer at all, and the `units` identity prerequisite is unmet. |
...
| 4 | **Coverage sweep** | Enquiries with `first_response_at IS NULL` past threshold; stage dwell past `target_time_in_status_hours` where set. Writes a `crm_task` and never releases an opportunity. | 50 rows |
| 4b | **Holding-period sweep (AUT-13)** | `assignment_state = assigned` and `assigned_at` older than `CRM_OWNERSHIP_EXCLUSIVITY` calendar days with no forward stage transition since, and no unexpired `ownership_extension`. Sets `assignment_state = warm_up` and writes the Owner a `crm_task`. **Never** writes `originating_owner_user_id` or a `crm_opportunity_credit` row, and never selects a successor. Owner-approved default — `docs/crm/CRM_JOURNEYS_AND_STATE_MACHINES.md` §7.3 | 50 rows |
...
| Signal | Source | Exact? |
|---|---|---|
| **Price** | `public.unit_price_history` — `id UUID`, `recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()`, `price_list_date DATE`. A new row **is** an event. | **For a new row, yes.** Composite watermark on `(recorded_at, id)`, storing no value. **The table is not append-only by construction** — the ingest can overwrite a price in place, which the watermark cannot see (§8.5, E3). |
| **Project status** | `public.project_status_history` — `id UUID`, `status TEXT`, `effective_date DATE`, `created_at TIMESTAMPTZ`. A new row **is** an event. | **For a new row, yes.** Same shape. [Repository fact] But **nothing writes this table** on `main`, so the signal cannot fire today (§8.7, V4). |
...
### 8.5 The `units` identity prerequisite — a second, independent blocker

Both exact signals are keyed to a canonical row, and §8.3 turns them into follow-up by joining that key to a watch set. **The join is only as sound as the identity underneath it, and for `public.units` that identity is half-settled.**

[Repository fact] `units.id UUID PRIMARY KEY DEFAULT gen_random_uuid()` is stable across re-ingest (`supabase/migrations/20260704055333_812d2f26-ad80-4807-b51a-bd3622cd5224.sql:79`) because the ingest resolves rather than recreates. But `unit_code` is `TEXT` and **nullable** (`:81`), **no `UNIQUE` constraint and no unique index on `(project_id, unit_code)` exists in any of the 25 tracked migrations** — the only indexes on `units` are the four non-unique ones at `:112-115` plus `idx_units_building_id` (`20260707101000_fdb001_inventory_facilities.sql:62-63`) — while the sibling inventory table already carries `UNIQUE (project_id, building_code)` (`20260707101000…:18`). The ingest nevertheless treats `(project_id, unit_code)` as the natural key with an unguarded SELECT-then-INSERT and no `ON CONFLICT` (`20260718113000_progressive_ingestion_v1.sql:669-670`, `:672-684`).

The blocker's full statement, the boundary rules it produces and the repoint-then-delete runbook are `docs/crm/CRM_PRODUCT_BOUNDARY.md` §3.3 and `docs/crm/CRM_DOMAIN_MODEL.md` §11. **This section adopts nothing new and states only what the defect does to *events*.**

**It does not disturb §8.2, and §8.2 does not absorb it.** `units_touched` stays suppressed for its own reason — `units.availability_status` is a mutable column with no history table — and would stay suppressed if the natural key became unique tomorrow. Conversely the two exact signals are exact *as records* whether or not duplicates exist, and are still not safely deliverable *to a person* until the identity under the watch set is settled. Two different defects, two different fixes, both gating unit-linked follow-up.

**Three ways the sweep can be wrong about a unit** [Inference, from the repository facts above]:

| # | Failure | Mechanism | What it looks like from outside |
|---|---|---|---|
| **E1** | **Split watch set** | Two concurrent `forever_progressive_ingest` calls can both miss on the resolving `SELECT` and both `INSERT`, producing two rows for one physical unit. A price row recorded against row A never joins to a person whose `focus_unit_id` is row B. | Silence. No error, no counter, no `needs_review` — the sweep runs clean and the buyer is simply never told. |
| **E2** | **Silent rebinding** | `SELECT id INTO v_unit_id` is a non-`STRICT` PL/pgSQL `SELECT INTO`: with two matching rows it does not raise, it assigns whichever row the plan returns first and discards the rest — https://www.postgresql.org/docs/current/plpgsql-statements.html | Follow-ups for one buyer stop mid-conversation when a later ingest binds the price stream to the other row. |
| **E3** | **In-place correction, invisible to the cursor** | [Repository fact] The ingest resolves an existing price row on `(unit_id, price_source, source_file, source_page, price_list_date)` compared with `IS NOT DISTINCT FROM` (`20260718113000…:729-734`) and, on a match, **`UPDATE`s that row in place** (`:750-761`), overwriting `price` and merging `metadata`. Only a miss inserts (`:736-748`). A `(recorded_at, id)` watermark advances on new rows only. | A corrected price produces no follow-up at all. |

**E3 corrects this package's own earlier wording.** §8.2 previously described `unit_price_history` as append-only; it is append-only *in the common path*, not by construction — the table carries `updated_at` maintained by `trg_unit_price_history_updated_at` (`20260707104000_fdb002b_unit_price_history.sql:47`) and there is no immutability trigger. [Repository fact] The price row's natural key *is* protected, unlike the unit's: `idx_unit_price_history_unique_source` is a `UNIQUE` index on `(unit_id, price_source, source_file, source_page, price_list_date)` (`20260707105000_fdb002c_import_modeva_units.sql:5-6`). [Inference] That guard is partial in exactly one direction — the index treats NULLs as distinct while the ingest matches with `IS NOT DISTINCT FROM`, so a tuple carrying a NULL `price_source`, `source_file`, `source_page` or `price_list_date` can still produce two rows the resolving `SELECT … INTO` then chooses between arbitrarily. E1 remains the more serious defect because `units` has no such index at all. Row identity is stable and `(recorded_at, id)` remains a correct watermark for **new records**; it is simply not a complete change feed. [Recommendation] A cursor that must also catch corrections would have to watch `updated_at`, which re-notifies on every metadata merge; that trade is recorded as open (§8.7, V7) and **neither option is adopted here**. Either way §8.4's fixed catalogue survives it unchanged — "the price record was updated" is true of an insert and of a correction alike, which is precisely why the title is not composed from values.

### 8.6 Unresolved unit interest, and how a change reaches a person without CRM-held unit truth

[Repository fact] Resolution is already fail-closed and canonical at `docs/crm/CRM_JOURNEYS_AND_STATE_MACHINES.md` §3.1 (J4): `WHERE unit_code = :raw AND project_id = :resolved_project`, and anything other than exactly one row leaves `focus_unit_id` NULL with the raw string preserved. §1.4 binds the other half — zero writes to `public.projects`, `public.units`, `public.developers` or any price table; `focus_*` resolve by lookup or to NULL, **never created**. What follows is the event-path consequence of those two rules, not a restatement of them.

| Capture outcome | How the interest is carried | Consequence in §8.3 |
|---|---|---|
| Exactly one matching unit row | `focus_unit_id` = that `units.id`; the raw inbound `?unit=` string is retained alongside it as capture evidence | In the watch set. A later price record for that unit reaches this person. |
| Zero matches, **or two or more matches** | `focus_unit_id` **NULL**; the raw `?unit=` string retained verbatim; `project_slug_at_capture` still recorded | **Not in the watch set.** No unit-level change follow-up will ever reach this person for that unit. Project-level interest may still be watched if the project resolved. |
| No resolved project to scope by | Unit resolution is **not attempted** — `unit_code` carries no cross-project uniqueness, so an unscoped lookup is meaningless | Neither watch applies. |

Three rules keep that fail-closed rather than fail-quiet:

1. **Never guess a unit, and never manufacture one to make the sweep fire.** Creating inventory so a follow-up has something to join to is exactly the invention of unit truth INV-D-1 forbids. [Repository fact] Note the deliberate asymmetry with the ingest, which **raises** on an unresolvable unit (`forever_progressive_ingest: price_unit_unknown`, `20260718113000…:724`) because `unit_price_history.unit_id` is `NOT NULL`: the ingest may refuse a batch, the CRM may not refuse a customer.
2. **A NULL link is a work item, not a silent gap** — an unresolved reference surfaces in the Owner console for a human decision and is never counted as unit-linked in any view (`docs/crm/CRM_PRODUCT_BOUNDARY.md` §3.3, B4). The manual equivalent in §9.5 is the compensating control: the workspace shows watched units with a "changed since" marker regardless of whether the sweep ran.
3. **A link resolved before the unique index exists is provisional.** Until E1/E2 are closed, "exactly one row" is not guaranteed even for a genuinely unique physical unit, so a resolution that succeeded is not by itself evidence that it bound to the right row.

**The delivery chain is entirely by key, and that is what keeps unit truth out of the CRM.** `unit_price_history.unit_id` → the watch-set join (`crm_person_interest`, open `crm_opportunity` focus, `crm_unit_hold`, `crm_reservation`) → `person_id` → one `crm_task` and one `crm_activity`. [Repository fact] `crm_job.payload` carries `{subject_kind, subject_id, person_ids, day_bucket}` and no values (§8.3); the handler and the UI re-read the canonical row at execution and render time. No unit column, no availability column and no price column is copied at any hop, so there is nothing the CRM holds that can go stale. What it holds is *"this person has not yet been told that a record changed"* — follow-up state, which §8.1 already assigns to the CRM.

The same discipline answers the retrospective question. **"What was it priced at when they enquired?" is a query, never a column** — read `unit_price_history` for the row current at the enquiry timestamp, on the server, under service_role. [Repository fact] `unit_price_history` is `REVOKE`d from `anon, authenticated` (`20260723130000_public_projection_privacy.sql:62`) because it carries `source_file`, `source_page` and provenance metadata, so that read never happens on a client-facing path. And because of E3 the answer is presented as **the current record for that source and price-list date**, never as an immutable historical quote. Storing "the price at the time" on the enquiry would be a second price of record with a stale value — the copy the boundary forbids.

### 8.7 What must be verified before unit-linked follow-up is enabled

[Recommendation] A gate list for the event path, in dependency order. Each item is checkable and none is a matter of opinion. The capability-by-capability split with owners is `docs/crm/CRM_PRODUCT_BOUNDARY.md` §3.3; this list is what the *sweep* needs. **Nothing here authorizes any of it**, and the sweep itself is Phase 2 at the earliest (§0.2).

| # | Verification | Class | Status today |
|---|---|---|---|
| V1 | `SELECT project_id, unit_code, count(*) FROM public.units WHERE unit_code IS NOT NULL GROUP BY 1,2 HAVING count(*) > 1` returns **zero rows** | Read-only, Slice-0 class | **Unknown.** Never run. If it is zero, the index work is genuinely additive and no data change is needed |
| V2 | A partial unique index on `(project_id, unit_code)` exists **with** its `WHERE unit_code IS NOT NULL` predicate. It must be an *index*: PostgreSQL offers the `WHERE` predicate only on `CREATE UNIQUE INDEX`, not on a table constraint — https://www.postgresql.org/docs/current/sql-createindex.html — so a rollback runbook saying "drop the constraint" does not execute | DDL state | **Absent** from all 25 tracked migrations |
| V3 | The repoint-then-delete runbook (`docs/crm/CRM_DOMAIN_MODEL.md` §11) was executed and recorded, or V1 proved it unnecessary. It is not optional cleanup: all three tables referencing `units(id)` do so `ON DELETE CASCADE`, so deleting a duplicate row first destroys that unit's price history in the same statement | Process | **Not started** |
| V4 | A producer exists for each watched table | Repository | **Half absent — verified.** `public.unit_price_history` is written by `forever_progressive_ingest` (`20260718113000…:736-748`, `:750-761`), so the price signal has a real producer. `public.project_status_history` has **no writer anywhere** — a repo-wide search of `src/`, `supabase/` and `scripts/` returns only DDL, grants, an index, an `updated_at` trigger, a policy, one generated-types entry and one test constant, and no `INSERT` or `UPDATE` against it — so the project-status half of §8.2 cannot fire. `public.price_updates` is in the same state and additionally carries `GRANT SELECT … TO authenticated` with RLS enabled and no policy, a grant that can never return a row (`docs/crm/CRM_CURRENT_STATE_AUDIT.md`) |
| V5 | The population of `units` rows with `unit_code IS NULL` is known, and the read path tolerates them without inventing a code. A partial index leaves them unconstrained **by design** | Read-only | **Unknown** |
| V6 | The ingest's resolving `SELECT` (`20260718113000…:669-670`) is re-read after V2 to confirm the planner can actually use the new index — one it cannot use still permits the race it was added to prevent | Code review | **Not done** |
| V7 | The cursor design's answer to E3 is recorded: watch new rows only and accept the correction blind spot, or watch `updated_at` and accept re-notification on every metadata merge | Design | **Open** |
| V8 | The prerequisite carries **its own ingest-subsystem task ID and its own migration timestamp later than `20260728120000`**, outside the six CRM filenames allocated in `docs/crm/CRM_IMPLEMENTATION_PLAN.md` | Governance | **To be recorded.** It modifies inventory truth and must not ride inside a CRM migration |

[Repository fact] Two constraints bind whoever eventually writes that migration. It is **not purely additive** — the forward file can fail on pre-existing duplicates, so the classification must say so and V1 must precede authoring. And it cannot use `CREATE INDEX CONCURRENTLY`, which cannot run inside a transaction block (https://www.postgresql.org/docs/current/sql-createindex.html): twelve of the twenty-five tracked migrations open with a literal `BEGIN;` and close with `COMMIT;`, and **every migration authored since `20260718113000` does so except `20260726120000_forever_direct_publish.sql`**, including the two most recent; zero migrations in the repository use `CONCURRENTLY`. [Unverified assumption] The stronger framing — that the Supabase runner wraps every file in a transaction regardless of contents — is not verified here and is not relied on; the repository convention is sufficient and is checkable. [Inference] The build therefore takes `ACCESS EXCLUSIVE` on `public.units` for its duration, which is negligible at present table size but must be recorded because `units` sits on the public project-detail read path.

[Recommendation] **V4 is the sharpest scheduling fact in this section, and it does not belong to the CRM.** Unit-linked *follow-up* is blocked by a missing writer in a subsystem the CRM does not own; unit-linked *opportunities and reservations* are blocked by V2/V3. Different blockers, different owners, tracked separately — merging them into one "units work" line hides the fact that no amount of CRM work moves either.
