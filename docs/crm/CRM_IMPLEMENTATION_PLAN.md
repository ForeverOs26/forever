...
7. **Merge is not apply.** `supabase db push` applies pending migrations in version order, so the first CRM apply necessarily replays the entire pending backlog first. The backlog is cleared as its own separately reviewed operation, never as a side effect of a CRM apply (§7.5).
8. **`public.leads` never gains a column.** Its `GRANT INSERT` is table-level and therefore extends automatically to any column added later, so widening the table is a privilege change wearing the costume of a schema change (§7.2, §8.3).
...
**Why zero new columns — the column-widening privilege hole.** This is the reason the rule above is stated as an exact column count rather than as a preference.

| # | Mechanism | Consequence for `public.leads` |
|---|---|---|
| 1 | [Repository fact] `supabase/migrations/20260704132000_create_leads.sql:29` reads `GRANT INSERT ON public.leads TO anon, authenticated;` — **no column list**, and **two** roles, not `anon` alone | The privilege is recorded against the relation, not against the twelve columns that existed on 2026-07-04 |
| 2 | [Web research] A table-level privilege covers every column of the table, including columns added later by `ALTER TABLE … ADD COLUMN`; the new attribute needs no column ACL because the relation-level entry already covers it — https://www.postgresql.org/docs/current/ddl-priv.html, https://www.postgresql.org/docs/current/sql-altertable.html | The instant a column is added, `anon` and `authenticated` may write it. No `GRANT` executes, no migration line changes, nothing appears in the diff |
| 3 | [Repository fact] The single INSERT policy's `WITH CHECK` constrains exactly four things — `status = 'new'` and non-emptiness of `name`, `email`, `phone`. RLS `WITH CHECK` is a row predicate, not a column allow-list | A column the predicate does not mention is unconstrained, not forbidden. The shipped policy is silent on every future column |
| 4 | [Web research] PostgREST accepts arbitrary column names in an insert body — https://postgrest.org/en/stable/references/api/tables_views.html; [Repository fact] `src/lib/lead-service.ts:92` inserts from the browser under the public anon key | The write surface is every column the role holds `INSERT` on, not the payload `lead-service.ts` happens to build. The caller is anyone |

[Inference] "The migration adds no `GRANT`" is a statement about migration text; the security question is a statement about database state, and mechanism 2 is exactly where the two come apart. A migration that adds columns to `leads` and calls itself "purely additive" is mislabelled: it is additive columns **plus an unstated privilege widening**. The acceptance criterion that follows from this is in §8.3; the security-side statement of the same rule is `docs/crm/CRM_SECURITY_AND_RBAC.md` §4.5, which refuses the `leads` → CRM foreign key on the adjacent existence-oracle ground. The two structural controls are the same one seen from two directions: the pointer lives on the internal side, and the public table never widens.
...
### 7.5 Replay, ledger drift and environment validity

The checks in §7.6 are mechanical. This subsection is the reason they exist: checks 2, 6 and 7 each answer a failure mode that is one command away in this repository today.

#### 7.5.1 The replay mechanic

| # | Statement | Label |
|---|---|---|
| MR-1 | `supabase db push` applies pending migrations **in version order**, and the CLI ledger is keyed on the leading `YYYYMMDDHHMMSS` version prefix — not on file content and not on file name | [Repository fact] `supabase/config.toml` contains exactly one line, `project_id = "abtvsrcnfwlbawvrjeed"`; nothing in this repository alters ordering, ledger keying or apply behaviour |
| MR-2 | Therefore **applying any CRM migration to production necessarily applies everything before it first** — the whole pending backlog, in one operation | [Inference from MR-1] |
| MR-3 | **Merge is not apply.** A CRM migration may merge into `main` while the pending backlog is unresolved. It may not be applied to production until that backlog is understood and its blockers are cleared by their own owners | [Recommendation] |
| MR-4 | The backlog is cleared **first, as its own separately reviewed operation**. At the time of the first CRM apply that is the first-ever application of the Studio, large-archive and direct-publish subsystem, including `supabase/migrations/20260724090000_studio_large_archive_v1.sql`, the largest migration in the repository. It is not a small operation and must never be presented as one | [Recommendation]; the file is a [Repository fact] |
| MR-5 | The **size** of the backlog is not a documentable constant. It is re-derived at apply time from a live read of `supabase_migrations.schema_migrations` diffed against `supabase/migrations/` (`FOREVER-CRM-004`). Any number written into a document is stale the moment another migration lands | [Recommendation] |

**Leading indicator.**

> Anyone describing the CRM apply as "one migration"; anyone reaching for `--include-all` to get past an out-of-order error

Both halves are diagnostic. The first means the speaker has not read the ledger. The second means the speaker is about to defeat the ordering guarantee — see §7.5.4.

#### 7.5.2 Version collisions and renaming hazards

| # | Rule |
|---|---|
| C-1 | A push meeting two files with the **same version prefix** either errors mid-run or records one version as done while applying the other. Both outcomes leave production in a state nobody can reconstruct from the ledger. [Inference from MR-1] |
| C-2 | **A CRM change must not resolve someone else's collision.** It belongs to whoever lands the colliding PR, with its own owner and its own review. A CRM migration that helpfully renumbers it makes the CRM packet answerable for another packet's merge. [Recommendation] |
| C-3 | The collision is nevertheless a **blocking dependency** of the CRM apply, not a note: recorded, not resolved. The live example and its resolution owner are §9's `#102` row. [Recommendation] |
| C-4 | Collision prevention: every CRM filename carries a timestamp strictly greater than the maximum across `main` **and every open PR**, which is why §7.1's floor is `20260728160000` (Draft PR #119) and not `main`'s `20260728120000`. Checking only `main` is the specific mistake that produced the duplicated `20260726120000`. [Recommendation] |
| C-5 | The floor is pinned by the `crm-migration-contract.test.ts` twin asserting the literal filename and its ordering, so the version cannot drift during review. [Recommendation] |
| RN-1 | **Never edit an applied migration.** Corrections layer as a later timestamped file. [Repository fact] The precedent chain exists: `20260721123000_studio_internal_acl_hardening.sql` → `20260722103000_studio_object_authorization.sql` → `20260722110000_studio_object_ownership_backfill.sql` → `20260722120000_studio_independent_review_corrections.sql`. Editing changes the file the ledger already records as done, so the edit never runs where it matters |
| RN-2 | **Never renumber a migration whose version has appeared in any ledger** — production, staging, or a developer's local project. Because the ledger keys on the version prefix (MR-1), a renumbered file presents as a *new pending migration* while the old version stays recorded as applied, and the DDL runs a second time against a database that already has it. [Inference from MR-1] |
| RN-3 | Renaming the **slug** while keeping the version is equally unsafe once applied: the contract test and any runbook step pinning the literal filename silently stops matching. [Recommendation] |
| RN-4 | Renaming is free in exactly one window — after the filename is chosen and before the first apply anywhere. Choosing the version against the correct floor (C-4) is what keeps that window wide enough. [Inference] |

#### 7.5.3 Migration text is not proof of live state

| # | Statement |
|---|---|
| L-1 | **The repository migration set is a proposal.** A `.sql` file under `supabase/migrations/` is a statement about a file, not about a running Postgres instance. [Recommendation] |
| L-2 | **An applied-migration figure copied out of a report is a point-in-time document, not a live read.** [Repository fact] `docs/FOREVER_STUDIO_PRODUCTION_PREFLIGHT_REPORT.md` records its applied-count observation as an observation made at report time, and migrations have landed on `main` since. Every pre-apply decision is taken against a fresh ledger read, never against the report |
| L-3 | The true pending set **and its order** come from `supabase_migrations.schema_migrations` diffed against `supabase/migrations/`. Divergence either way is a stop condition: a ledger row with no matching file means something was applied that is not in the repository; a file with no ledger row is pending |
| L-4 | The design must not require production schema that does not exist yet, and the CRM migration must be assumed to sit unapplied for some time. Applying it is a **separate Owner checkpoint**, not a consequence of a merge. [Recommendation] |
| L-5 | **Never write "the gate passes" about this repository.** [Repository fact] There is no `.github/`, so nothing runs automatically. Every migration check is enforced only by a named person choosing to run the command and reporting the result honestly, on a named date. A ledger check nobody runs is not a control |

#### 7.5.4 Environment validity, and two tests that are not substitutes

[Recommendation] Not every database is a valid target for developing or testing a CRM migration. A database holding a **back-dated** migration — a file whose version sits below versions already on `main` — is not one: pushing the merged set from there produces the out-of-order insert the CLI refuses. The pressure response to that refusal is `--include-all`, and that is the trap, because it applies files **below the recorded high-water mark**, which can land a constraint relaxation or a destructive statement *after* objects created under the stricter contract. A frozen pilot or staging project pinned to a back-dated migration is therefore explicitly not a valid CRM test target, however convenient it is; the environment a CRM migration is developed against must have the full `main` backlog applied in version order first.

| Test | What it runs | What only it can prove | Harness |
|---|---|---|---|
| **Clean-environment** | The full committed chain **plus** the new files, from an empty cluster, in real version order | That the chain applies at all: FK and dependency order within and across files, extension availability, syntax, grant and RLS posture as written | [Repository fact] `npm run studio:pg-test` → `scripts/studio/run-postgres-tests.mjs` with `scripts/studio/pg-bootstrap.sql`, on a disposable PostgreSQL |
| **Upgrade-path** | **Only the new files**, on a database already at the current applied high-water mark, **with rows present** | Everything a clean run structurally cannot fail on: a `NOT NULL`/`DEFAULT` addition rewriting existing rows; a unique or CHECK constraint that fails on existing data; a backfill's real match and ambiguity rate; lock behaviour and duration on a populated table; ordering against a real remote ledger | **None exists on `main`.** [Repository fact] Draft PR #102 contributes `scripts/studio/run-migration-upgrade-test.mjs`; it is on Draft footing and must not be assumed available |

[Inference] The asymmetry, stated plainly: a clean-environment run cannot fail on existing duplicate rows, cannot detect ledger drift and cannot measure a backfill; an upgrade-path run cannot prove the chain builds from zero. Passing one is not evidence about the other, and reporting "the migration tested clean" without naming which test ran is the reporting failure this subsection exists to prevent. `CRM-026`'s idempotence claim and `CRM-022`'s `pg_trgm` dependency sit on opposite sides of that line.

**Transaction-wrapper consequence.** [Repository fact] Supabase wraps each migration file in a transaction. Two consequences belong to apply-time correctness rather than to style: `CREATE INDEX CONCURRENTLY` is unavailable, so an index build holds `ACCESS EXCLUSIVE` on its table for the duration; and one failing statement rolls back every other object in the file, so a single missing dependency loses the whole file's work. That is why the three-file FK ordering in §7.1 is an apply-time property and not a reading convenience.

### 7.6 Read-only pre-apply check
...
| 2 | **Ledger** — `supabase_migrations.schema_migrations` diffed against `supabase/migrations/`; the pending set and its order recorded. **The pending backlog is cleared as its own reviewed operation before any CRM file is pushed** (§7.5.1) | Drift, or an unresolved backlog. Stop until `FOREVER-CRM-004` reconciles it |
...
| 6 | **Name collision** — no existing object named `crm_*`; every filename strictly above the current high-water mark **across `main` and every open PR**, checked before the version has entered any ledger (§7.5.2) | Rename before applying — and only while renaming is still free |
| 7 | **Dry run** — `npm run studio:pg-test` applies the full committed chain plus the new files on a disposable cluster. This is the **clean-environment** test and proves nothing about existing rows or ledger order; the environment it runs against must hold no back-dated migration (§7.5.4) | The chain does not apply cleanly in real order. Stop |
| 7b | **Upgrade-path observation**, where the change touches populated tables — the new files only, on a database already at the applied high-water mark, with rows present | No harness exists on `main`. Record the absence as a stated gap; never report a clean-environment pass as if it were this |
...
| 10 | **Worker rollback target recorded** — the current deployed version id | See §7.7. This is the weakest link and must be recorded even when it cannot be exercised |

### 7.7 Rollback and restore truth

> **A rollback table is only honest if someone can execute it.**

Four rules make the table below a capability rather than a claim. [Recommendation] (1) Where a reverse step depends on **text being preserved** — an original `GRANT`, an original policy `WITH CHECK` — that text is reproduced verbatim in the migration's own header comment; the preservation is part of the rollback, not a footnote to it. (2) Where a reverse step depends on **a runbook having been run first**, the runbook is named in the same row as a precondition. (3) A rollback that has never been rehearsed is a claim, and [Repository fact] there is no `.github/` here to catch one that never was. (4) Reverse steps that are technically possible but **never wanted** are labelled as such rather than omitted — a privilege-tightening reverse that re-opens a hole is written down precisely so nobody performs it by reflex while reversing something adjacent (§8.3's `leads` criterion is exactly such a case).
...
**Restore is the reverse step nobody writes down.** [Inference] A default point-in-time-recovery or snapshot configuration retains deleted rows well past the erasure window, so "we ran a `DELETE`" is not compliance and **a restore can silently resurrect erased personal data**. [Web research] Thai PDPA deletion rules require erasure to reach personal data including its copies and backups within **90 days**, with interim protective measures where that is not achievable, and require a verification system — https://www.lawplusltd.com/2024/08/rules-on-deletion-destruction-or-anonymization-of-personal-data/

| Option | Consequence |
|---|---|
| **A. Keep PITR / snapshot retention under 90 days** | Simplest and self-enforcing; reduces disaster-recovery depth — corruption discovered on day 100 is unrecoverable |
| **B. Longer retention plus an erasure ledger replayed against any restored snapshot** before the database is accepted back into service | Preserves DR depth; requires a restore runbook that exists and has been rehearsed. **A restore performed under pressure will skip it unless it is automated** |
| **C. Longer retention and no procedure** | Not defensible |

[Recommendation] **Option A plus a documented restore-time ledger check** — a light form of B — is proportionate at this scale. **[LAWYER]** to confirm the 90-day scope and whether interim protective measures are acceptable for backup media. **Descriptive only, not legal advice.** The decision is free to make now and expensive to make after the first production restore: deciding it while no CRM row exists is the cheapest it will ever be, which is the same reasoning that puts the irreversibility boundary at the **first write** rather than at the migration.
...
| `CRM-024` | `public.leads` compatibility guard: `BEFORE UPDATE` status-freeze trigger and two corrected `COMMENT`s. **Zero new columns** | `CRM-022` | **R2** — touches the one public write contract | register row 6 | A `service_role` `UPDATE` of `leads.status` is rejected; the table comment reads "Public intake mirror. Not complete…"; the migration text contains no `ADD COLUMN` against `public.leads`, pinned by the contract twin (§7.2, §8.4) |
...
### 8.4 The standing acceptance criterion for any future widening of `public.leads`

This package adds **zero columns** to `public.leads` (`CRM-024`, §7.2), and §7.2's four mechanisms are the reason. The rule is recorded as a standing constraint rather than as remediation work, because the package is safe by *construction* and construction is a decision one later contributor can reverse in a single line without noticing.

1. **`public.leads` is the intake log.** It never accretes CRM state — no linkage, no provenance, no assignment, no ownership or attribution, no workflow columns, no free-form `JSONB`. The one-directional `crm_enquiry.legacy_lead_id` pointer exists so that no such column is ever needed. Neither structural control — the one-way pointer with no FK, and the zero-column rule — may be relaxed without re-running §7.2's analysis.
2. **Any proposal to add any column to `public.leads` is reviewed as a privilege change**, and is accepted only if the same migration file also (a) issues `REVOKE INSERT ON public.leads FROM anon, authenticated;` **before** granting anything back, so an interrupted apply fails closed; (b) re-grants column-scoped over the intake columns only, in the idiom this repository already uses — [Repository fact] `supabase/migrations/20260723130000_public_projection_privacy.sql:19-29` does exactly a column-less `REVOKE SELECT` followed by a column-enumerated `GRANT SELECT` on `public.projects`, and repeats it for five further tables; (c) states honestly whether the re-grant preserves or **narrows** the anonymous surface — omitting `id` and `created_at` narrows it, because today `anon` can supply its own primary key and its own `created_at`, and a migration may not simultaneously narrow and claim net capability is unchanged; and (d) restates the constraint in the RLS policy as an independent backstop, carrying the original four `WITH CHECK` conjuncts verbatim from `20260704132000_create_leads.sql:32-41` plus one conjunct per new column, since PostgreSQL has no `ALTER POLICY … ADD` and the predicate is what still fails closed if a later migration carelessly re-grants at table level.
3. **The proof is executed against a real PostgreSQL instance, not read off a diff.** Text-pinning cannot reach this defect, because the defect is the *absence* of a statement. Under `npm run studio:pg-test`, for **both** `anon` and `authenticated` — a probe set that checks only `anon` proves half the statement, since line 29 grants to two roles:

| Probe | Expected | What it catches |
|---|---|---|
| `has_column_privilege(<role>,'public.leads','<new_column>','INSERT')` | `false` | The core assertion. [Web research] The right probe precisely because it returns true when the privilege is held at **either** column or whole-table level — https://www.postgresql.org/docs/current/functions-info.html — so it detects the silent table-level inheritance a column-ACL inspection would miss |
| `has_column_privilege(<role>,'public.leads','name','INSERT')` | `true` | That the tightening did not break the live intake path |
| `has_table_privilege(<role>,'public.leads','INSERT')` | `false` | That the table-level grant is genuinely gone. Also the check for the `REVOKE`-by-a-non-grantor case: [Web research] `REVOKE` removes only grants made by the current role, so it can succeed syntactically and remove nothing — https://www.postgresql.org/docs/current/sql-revoke.html |
| `pg_attribute.attacl` for the new column and `pg_class.relacl` for `public.leads` | column ACL names only the intended roles and privileges; the relation ACL no longer carries `a` for `anon`/`authenticated` | Distinguishes the two ACL levels directly. **Not sufficient alone** — before remediation the new column's `attacl` is `NULL` while `anon` can still write it, which is why the privilege probes lead and this one corroborates |
...
| Behavioural, as `anon`, with the grant temporarily broad inside a rolled-back transaction | fails on the policy | Proves control (d) works independently of control (b) |

4. **"No new `GRANT` was added" is recorded here as insufficient evidence**, and so are "the RLS policy is unchanged" (an unchanged predicate silently fails to constrain a column that did not exist when it was written) and "the application's payload does not include the new column" (irrelevant — the anon key ships in the browser bundle, so the caller is not the application). The only admissible question is *what can `anon` write after this migration that it could not write before*, and it is answerable only by probing a database. [Repository fact] There is no CI, so no probe result may be described as a gate that passed — only as an observation a named person made on a named date.
5. **A column-level `GRANT INSERT (` is a different statement from a column-level `GRANT UPDATE (`.** §8.3 asserts zero occurrences of the latter and nothing above proposes, requires or permits it on any table.
6. **Reverting a tightening is never done alone.** Restoring the table-level grant reopens the hole for every column added since, so such a revert happens only together with dropping the columns it protects.

[Recommendation] A proposal that omits items 2 and 3 is rejected on that ground alone, without reaching its merits.
...
| 7 | **The migration chain does not apply** — version collision, wrong apply order, or a forward foreign-key reference. #102 collides at `20260726120000`; #119 holds `20260728160000`; `20260723130000` is intentionally unapplied and strips column grants if applied late; and a forward FK is the defect that broke the pre-review draft | M | H | Two files share a timestamp; `studio:pg-test` aborts; a grant disappears after apply | §7.1's register; §7.5.2's collision and renaming rules — the floor is checked against every open PR, not only `main`, and a version that has appeared in any ledger is never renumbered; pre-apply steps 2, 3, 6 and 7; `CRM-009` settles #102 first. The three-file split is FK-ordered by construction: catalogue, then identity, then timeline with `crm_activity` before `crm_consent_event` | Architect | **Yes** |
...
| 23 | **The first CRM apply replays the entire pending migration backlog.** `supabase db push` applies pending files in version order, so pushing a CRM migration necessarily applies everything before it first — at that moment the first-ever application of the Studio, large-archive and direct-publish subsystem, including `20260724090000_studio_large_archive_v1.sql`, the largest migration in the repository. Announced as "applying the CRM migration", it is an unreviewed production change of a completely different size | M | **H** | *Anyone describing the CRM apply as "one migration"; anyone reaching for `--include-all` to get past an out-of-order error* | §7.5: merge is not apply, and the backlog is cleared first as its own separately reviewed operation under its own owners. The pending set is re-derived from a live `supabase_migrations.schema_migrations` read at apply time (`CRM-004`), never from a document, a report or a remembered count. `--include-all` is never used, because it applies files below the recorded high-water mark. The environment a CRM migration is developed against must hold no back-dated migration | Owner + Architect | **Yes** |
