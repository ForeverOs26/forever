...
**[D] Owner currency clarification (2026-07-20):** every current Forever project is in Thailand. An explicitly stated selling-price currency is preserved exactly as `source_verified`, including an explicit non-THB currency. When no selling-price currency is stated, the existing currency policy applies THB as `inferred_default`; this approved default does not become source evidence and does not require an exception review item. This clarification supersedes earlier SIP-001 wording that left missing currency unresolved or automatically queued every inferred currency.
...
| Price-list cell        | `Fact<T>`, used in `ExtractedPriceListRow`                                                 | SIP emits `high`, `medium`, `low`, `none`. Exact `Fact.status`: `source_verified`, `inferred_default`, `unresolved`, `conflict`; never `extracted`                                                                                                | `source_file`, `page_number`, optional `sheet_name`; `source_row` is on row. Use null + none for a present unavailable cell; omit absent field | Medium/low, recovery, ambiguity, conflict, and duplicate identity review; approved default THB is not an exception | Invalid facts nulled; invalid price/currency warned; missing unit identity skipped          |
| Currency               | `CurrencyDecision` in `src/import/currency-policy.ts`                                      | Status: `source_verified`, `inferred_default`, `unresolved`, `conflict`; confidence: `high`, `medium`, `none`                                                                                                                                     | Null value for unresolved/conflict; `CurrencyEvidence` holds file/page                                                                         | Conflicting or malformed explicit evidence requires review; approved missing-currency THB default does not         | `decideCurrency` owns the Owner-approved Thailand/THB default                               |
...
9. Normalize known availability labels only with a versioned fixed mapping; unknown label is retained and reviewed. Extract date only from document content. Preserve explicit currency only from evidence applicable to the selling-price column; fee labels are not price-currency evidence. If that currency is absent, apply THB through the existing Owner-approved Thailand default as `inferred_default`.
...
- Medium/low, ambiguous headers, conflicting values, duplicate identities, unclear dates, and parser recovery require review. The Owner-approved missing-currency THB default is the explicit exception and remains `inferred_default`.
- Accept retains actual status: normal source-exact price fact remains `source_verified`, not `owner_verified`.
- Edit records original and edit; it is not owner verification. SIP price-list `Fact` does not emit `owner_verified`.
