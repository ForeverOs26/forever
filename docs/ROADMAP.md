...
RC5.5 Coralina safe execution, currently limited to RC5.5D live execution boundary preparation. See `docs/CURRENT_STAGE.md`.

RC5.4 is closed as Completed: the shared Project Knowledge Platform reports Coralina `ready`, and the Import Engine dry-run plans 405 operations with zero writes. RC5.5A is closed as Completed and merged: deterministic plan fingerprints and fail-closed, non-networked target preflight. RC5.5B is closed as Completed, merged, and locally proven: the Owner's read-only proving run against the reconciled canonical local target reported Coralina as 405 `absent` operations with zero collisions. RC5.5C is closed as Completed, reviewed, integrated, and merged: hermetic transaction/rollback preparation with the live runner disabled. RC5.5D adds only server-side/live-boundary preparation — a committed-but-NOT-applied migration, durable approval storage, a bounded server request contract, an isolated credential boundary, an effective-privilege & PUBLIC-ACL audit contract, and a disabled live adapter over a single fixed direct-PostgreSQL statement; no real import has occurred, real database writes remain zero, and live execution stays disabled.
...
- RC5.5 Coralina safe execution
  ...
  - RC5.5D (current, server-side/live-boundary preparation only): a committed-but-NOT-applied migration defining one atomic server-side execution function with durable digest-only approval consumption and durable receipts under a locked-down privilege model, the callable wrapper in a dedicated closed `forever_execution` schema (not `public`); an effective-privilege & PUBLIC-ACL audit contract that measures the executor's effective privileges and inspects the PUBLIC grantee through ACL catalogs (a direct REVOKE cannot override PUBLIC), classifying any PUBLIC-derived public-schema USAGE as blocking pending Owner reconciliation (option B), plus ownership and sequence checks — with table/sequence/CREATE/USAGE/SECURITY-DEFINER checks spanning the entire non-system database surface (EXECUTE on any routine, explicit per-schema classification, and complete execution-chain binding to one exact dedicated NOLOGIN owner role (forever_import_execution_owner) with exact boundary ownership, an exact capability allowlist, positive required-capability proof, and owner-scoped RLS SELECT/INSERT policies deterministically normalized (DROP-and-recreate of ten dedicated per-table policies, never IF NOT EXISTS) and proven exactly at runtime (exact name/table/command/PERMISSIVE/role-set/constant-true expressions, PUBLIC applicability via polroles OID 0, full applicable-policy classification failing closed on drift) without BYPASSRLS or target-table ownership) (every schema not on the explicit system allowlist), so Supabase/extension schemas are audited and the approved wrapper is the sole allowed privileged callable surface; a typed versioned bounded server request contract; an isolated execution credential boundary; and a typed live adapter over a single fixed direct-PostgreSQL statement, disabled behind a false-by-default capability plus an explicitly injected transport. The migration is not applied; no Supabase connection is made; approval issuance and the first real import remain separate Owner checkpoints.
  - Migration application, real approval issuance, the supervised first Coralina import, staging rehearsal, and the first permanent Coralina write remain later explicit Owner checkpoints.
