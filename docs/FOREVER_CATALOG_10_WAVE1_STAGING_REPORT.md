...
Branch: the superseded Catalogue 10 working branch (since deleted)
Worktree: a dedicated task worktree (local path not recorded)
Base SHA: `a9d275fc678065ef70b331aee20f24f1c4f030e6`

> ## Read this first — the record below is historical
>
> **This document reports what happened on 2026-07-26. Two payloads have been
> corrected since, so parts of it no longer describe the committed files.** The
> figures are deliberately left as recorded: editing them would falsify the
> account of what was actually validated and loaded.
>
> **What changed.** An independent audit found that The Title Sierra's currency
> evidence cited page 4 of a document with three pages, and that Layan Green
> Park's descriptive fields borrowed a price-validity date the source does not
> state for them. Both were corrected and both payloads were rebuilt.
>
> | Payload            | Recorded below (imported)    | Committed now  |
> | ------------------ | ---------------------------- | -------------- |
> | `the-title-sierra` | fingerprint `4a3e9c17d…`     | `53f569ffe…`   |
> | `layan-green-park` | fingerprint `288fbca7d…`     | `396feb209…`   |
>
> `coralina`, `rainpalm-villas`, `garden-of-eden` and
> `ayana-heights-seaview-residence` are unchanged, and every figure recorded for
> them still matches the committed files exactly.
>
> **The consequence, stated plainly.** The Sierra draft held in staging is the
> **pre-correction** payload. Staging was **not** re-imported, and **no staging
> database was contacted** by the work that made these corrections. Bringing
> staging into line would require a fresh, separately authorised import that has
> not been performed. Layan was never imported anywhere, so nothing diverges
> there — only its committed payload changed.
...
the superseded branch head that was audited and imported. All four projects are present as
unpublished drafts.
...
| Continued from | the superseded planning commit — planning-only commit, unpushed at task start                            |
| Worktree       | a dedicated task worktree; the primary repository clone was read but never modified |
...
| `.env`                                               | Only one exists, at `.env` in the repository root. It holds a **production** URL and a publishable (anon) key. No staging URL, no staging key, no service-role key.                                                         |
...
Each project is one command, run from the task worktree in an
...
| Production credential used     | **None.** The production anon key in `.env` in the repository root was never read into any command. |
...
| Check    | Result                                                                 |
| -------- | ---------------------------------------------------------------------- |
| PR head  | the superseded trust-policy commit — matches the expected head |
| Branch   | the superseded Catalogue 10 working branch (since deleted)             |
...
| `.env`                                               | only `.env` in the repository root exists; production URL and publishable key only |
...
the task worktree, with the staging host and staging CA set
...
## 14. Independent local readiness audit at head the superseded branch head audited at that time
...
| Item                        | Value                                                                   |
| --------------------------- | ----------------------------------------------------------------------- |
| Head audited                | the superseded branch head audited at that time                          |
...
> **Recomputed 2026-07-26 for the sanitized reconstruction branch.** The figures
> below were re-measured against that branch; they are not carried over from the
> superseded one. Every hand-written file this change adds — the four documents,
> the builder and the test file — is Prettier-clean. The remaining drift is
> entirely generated payload output.

| Scope                                    | Merge-base | Head |  Delta |
| ---------------------------------------- | ---------: | ---: | -----: |
| Files differing from Prettier, repo-wide |        129 |  133 | **+4** |
| …of those, inside `forever-data/`        |          2 |    6 |     +4 |

The four are the generated payloads this change writes: Garden of Eden, Layan
Green Park, Rainpalm Villas and The Title Sierra. (AYANA's payload happens to
serialise Prettier-clean; the archived Rainpalm `payload.fast-intake-v1.json` is
byte-identical to a file that was already tracked, so it adds no new drift.)

`JSON.stringify(value, null, 2)` breaks short arrays across lines where Prettier
would inline them — specifically the `applies_to` arrays introduced by the trust
policy's row-level source records. Reformatting changes only whitespace: the
parsed content is identical and **every `batch_fingerprint` is unaffected**,
because the fingerprint is computed over a key-sorted canonical serialisation,
not over file bytes.

It was left as-is deliberately, and the reason is stronger than cosmetics: the
builder's `--check` mode compares **raw text**, so running `prettier --write`
over a generated payload makes `--check` report `DIFFERS` and exit 1. Formatting
the generated output would break the determinism guarantee it exists to prove.
The repository does not hold this tree to Prettier cleanliness in any case — 129
files already differ at the merge-base, two of them inside `forever-data/` — so
no existing invariant is broken. The alternative fix, making the builder format
its own output through Prettier, would couple a currently dependency-free script
to `node_modules` and lose its ability to run on a bare checkout.

### 14.5 Structural checks

> **Recomputed 2026-07-26 for the sanitized reconstruction branch.** The earlier
> figures in this row described an intermediate head and were stale by the time
> that branch was closed.

| Check                                             | Result                                                                                                                 |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Files changed versus `main`                       | **13** — one `.gitignore`, four docs, six payload JSONs, one builder script, one test file                              |
| Runtime source (`src/`) changed                   | **none** — the only `src/` addition is `src/intake/tests/catalog-wave1-builder.test.ts`, a test                        |
...
| `.gitignore` sentinels                            | `intake/`, `sip/` and `source/` sentinels ignored; exactly the six payloads allowed                                     |
| Executable, archive or media files added          | none — only `.json`, `.md`, `.mjs` and one `.ts` test                                                                  |
| Source PDFs, extracted text or temp artefacts     | **none tracked**                                                                                                       |
...
| Repository head | the superseded branch head that was audited and imported — the head audited in §14 | in the record                                                                             |
| Branch          | the superseded Catalogue 10 working branch (since deleted)           | in the record                                                                             |
