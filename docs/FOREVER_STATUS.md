...
Coralina's production draft import is completed as a draft only: 1 project, 8 buildings, 198 units, 198 prices, 6 warnings, and 1 ingestion batch. Coralina remains unpublished; publication is a separate, later action. Fast Intake v1 with a 15-minute draft target is implemented and independently Windows-validated in the open, unmerged PR #85 (pending Owner merge; not yet canonical on `main`); no production execution occurred, Coralina remains unpublished, and Factory autonomy remains A0.

Fast Intake v1 is a bounded, local, owner-only preparation-and-validation tool (`npm run intake`; on Windows, `npm.cmd run intake`) that turns project source materials (a folder and/or ZIP archives) into a deterministic, validated, unpublished Progressive draft payload for the ordinary generic draft importer. It reuses the existing Progressive builder, deterministic fingerprint, currency policy, provenance/warning model, and the ordinary `-ValidateOnly` invariants, with a hardened untrusted-ZIP boundary, journaled crash-recoverable transactional artifact output, per-project locking, and strengthened anti-fabrication guards. It creates no database client, makes no network request, performs no production write (its only writes are local generated artifacts and a gitignored temporary workspace), performs no publication, and adds no schema, migration, RPC, RLS, grant, backend service, or UI.
