...
Fast Intake v1 is a bounded, local, owner-only preparation-and-validation tool (`npm run intake`) that turns normal project source materials (a folder and/or ZIP archives) into a deterministic, validated, unpublished Progressive draft payload for the ordinary generic draft importer. It reuses the existing Progressive builder, deterministic fingerprint, currency policy, provenance/warning model, and the ordinary `-ValidateOnly` invariants. It creates no database client, makes no network request, performs no write, performs no publication, and adds no schema, migration, RPC, RLS, grant, backend service, or UI. See `docs/FAST_INTAKE_V1.md`.
