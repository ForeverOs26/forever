...
Structured Input Preparation Design v1 is independently reviewed, Owner approved, and canonical. SIP-001 starts with local deterministic extraction from qualified text-based PDF price lists and keeps existing Fast Intake contracts and safety boundaries unchanged. The active development checkpoint is SIP-001A: qualify and deterministically extract one supported text-based PDF price list into candidate `ExtractedPriceList` JSON, validate it, produce an exception-only review summary, and prove current Fast Intake consumes the reviewed result unchanged. Rainpalm is the intended first pilot only after its actual raw price-list PDF is confirmed in an authorized local source folder; the committed payload is comparison ground truth but does not prove that raw PDF is present. Its absence blocks the real Rainpalm pilot, not safe-fixture infrastructure work. Partner Demo v1 remains canonical and presentation-ready, pending partner scheduling in parallel; it does not block SIP-001A. Coralina remains unpublished, Rainpalm remains unimported and unpublished, and Factory autonomy remains A0. See `docs/STRUCTURED_INPUT_PREPARATION_DESIGN_V1.md` and `docs/FAST_INTAKE_PILOT_01_RAINPALM.md`.
...
The Owner-authorized Coralina draft import through the generic importer is completed as a draft only; publication remains separately authorized. Shared Navigator (website + Booth Mode), Fast Intake v1, Partner Demo v1, and Structured Input Preparation Design v1 are canonical on `main`. The active product-development checkpoint is SIP-001A, limited to authorized local raw price-list PDF input; existing Fast Intake safety-boundary reuse; local Poppler preflight; text-layer qualification; one supported deterministic table layout; exact page/row references; normalization; duplicate/ambiguity blocking; exception-only CLI review; reviewed final `ExtractedPriceList` JSON; deterministic repeat; and unchanged Fast Intake compatibility. Project-facts automation, XLSX/CSV, OCR, scanned PDFs, images/floor plans, AI/cloud extraction, database connection, import, publication, admin UI, and Factory autonomy expansion are excluded. Partner Demo v1 presentation remains pending in parallel and does not block SIP-001A.
...
- The real Rainpalm SIP-001A pilot is blocked until its actual raw price-list PDF is confirmed in an authorized local source folder; safe-fixture qualification/parser infrastructure may proceed.
