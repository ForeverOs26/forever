...
RC5.5 Coralina safe execution, currently bounded to RC5.5C Transactional Execution and Rollback Preparation. RC5.4, RC5.5A, and RC5.5B are closed as Completed and merged; RC5.5B is additionally locally proven — the Owner's read-only proving run against the reconciled canonical local target reported Coralina as 405 `absent` operations with zero collisions. RC5.5C is implemented as preparation only and is pending Owner review and integration; no real import has occurred and real database writes remain zero. See `docs/CURRENT_STAGE.md`.
...
- RC5.5B Read-Only Target Collision Inspection (completed, merged, locally proven): an explicit `--inspect-collisions` mode that, only after a successful RC5.5A local preflight, uses a narrow select-only `CollisionInspectionReader` (never the mutation-capable `DatabaseLayer`) with proven-complete exact-count pagination to read only the plan-referenced project, buildings, units, and price-history rows and classify each operation as `absent`, `exact_match`, `update_required`, `duplicate_target_rows`, `identity_conflict`, or `inspection_error`. Dry-run stays client- and network-free, incompatible mode combinations fail closed, and every report states `readOnlyConfirmed: true`, `executeEnabled: false`, and `writesPerformed: 0`. The Owner's local read-only proving run against the reconciled canonical local target reported Coralina as 405 `absent` operations (1 project, 8 buildings, 198 units, 198 price-history rows) with zero collisions, duplicates, identity conflicts, or inspection errors.
- RC5.5C Transactional Execution and Rollback Preparation (implemented as preparation only, pending Owner review and integration): a single-transaction execution boundary with a narrow typed mutation surface (no generic table access, raw SQL, RPC, or delete), an Owner execution-approval artifact contract (scope-bound, short-lived, single-use, fail-closed), deterministic transaction ordering (dependencies read first, then project → buildings → units → price history), in-transaction verification of every persisted row against the shared RC5.5B persistence projections before commit, automatic rollback with deterministic sanitized reason codes on every failure category, a typed deterministic execution receipt, a strict fail-closed repeat-execution boundary (only a proven-fresh all-`absent` target is executable; in-transaction state drift rolls back), and an explicit `--execute-approved-import` CLI mode that fails closed before any transaction, database client, network access, or approval consumption. The live transaction runner is structurally defined but disabled (`live_execution_disabled`); no real import has occurred; real database writes remain zero; the first real Coralina import remains a separate Owner checkpoint.
