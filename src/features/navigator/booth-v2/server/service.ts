 ...
 *
 * SERVER-ISSUED SESSION IDENTITY (corrective pass 5). `createSession` is the
 * ONLY operation that brings a session into existence, it takes NO input, and
 * the reference it returns is minted by the database. Nothing else creates:
 * `ensureSession` opens an existing session and refuses an unknown or a foreign
 * reference identically, changing zero rows either way.
 *
 * That is also why no operation here pre-calls `ensureSession` any more. Each
 * transition RPC already opens the session through `booth_lock_owned_session`,
 * which proves existence and ownership before any write and raises the same two
 * exceptions this boundary collapses — so a pre-call could only produce an
 * identical refusal one round trip earlier. Now that ensure no longer creates,
 * keeping it would have been pure redundancy; removing it also means a rejected
 * operation touches the database exactly once.
 *
 ...
 *
 * The ONE operation that brings a Booth session into existence, and the only
 * place a client reference is ever minted. It takes NO input at all: the
 * reference is generated by the database (gen_random_uuid), the Host is the
 * authenticated actor, and the booth id is server configuration. The browser
 * therefore cannot choose the identity of a new session, cannot guess an
 * existing one, and cannot cause a session to exist under a reference it
 * invented.
 *
 * One invocation creates exactly one session and persists nothing about a
 * guest — only the operational shell the pre-consent CHECK permits.
 */
export async function createSession(actor: BoothActor): Promise<{ clientRef: string }> {
  // The demo no-write mode still returns a SERVER-issued reference; it is
  // simply never persisted, exactly as every other write is skipped.
  if (isBoothDemoNoWriteMode()) return { clientRef: crypto.randomUUID() };
  const config = await getBoothConfig(actor);
  const clientRef = await callRpc<string | null>("booth_create_session", {
    p_host_user_id: actor.userId,
    p_host_email: actor.email,
    p_booth_id: config.boothId,
  });
  // A create that did not return a usable reference is a failure, never a
  // silent success the tablet would then replay against nothing.
  if (typeof clientRef !== "string" || clientRef.trim().length < 8) {
    console.error("[booth-v2] create session returned no usable reference");
    throw new BoothError(
      "booth_request_failed",
      "The booth hit a temporary problem completing this step. Please try again.",
    );
...
/**
 * Open an EXISTING session the acting Host already owns. This CREATES NOTHING
 * (corrective pass 5): it used to INSERT for an unknown reference, which let an
 * authorized staff account tell an unused reference apart from one owned by
 * another Host — and mint arbitrary empty sessions from invented references.
 *
 * An unknown reference and a foreign reference now produce the SAME external
 * refusal (see boothSessionUnavailable), and neither creates or changes a row.
 * A session the acting Host owns succeeds idempotently, including when it has
 * already reached a terminal outcome — the RPC only reads.
 */
