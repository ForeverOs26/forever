/**
 * Forever Project Readiness — deterministic, immutable helpers.
 *
 ...
 * stats combiners are the RC4.0 ones under readiness names — nothing is
 * restated.
 */
...
// a readiness name — the module's one string-ordering rule, never bent to the
// host locale and never a local variant.
export { compareCrossValidationStrings as compareReadinessStrings } from "@/features/forever-cross-validation";

// Reuse the RC4.0 stats combiners (through the RC4.6 re-export — the very
// same functions) under readiness names — the stats shape is the RC4.0 one,
// so the arithmetic is too.
export {
  mergeProjectDatabaseStats as mergeReadinessStats,
  sumProjectDatabaseStats as sumReadinessStats,
