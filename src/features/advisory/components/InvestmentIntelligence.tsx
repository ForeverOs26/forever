import { NOT_AVAILABLE, type InvestmentIntelligence } from "../investment-intelligence";

/**
 * InvestmentIntelligence — foundation section for the Advisory Workspace.
 *
 ...
 * never renders a numeric investment score.
 */
export interface InvestmentIntelligenceProps {
  data: InvestmentIntelligence;
  headingId?: string;
}

const VERDICT_BORDER: Record<InvestmentIntelligence["readinessVerdict"], string> = {
...
export function InvestmentIntelligence({
  data,
  headingId = "advisory-investment-intelligence-heading",
  ...
  return (
    ...
    >
      ...
      <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Entry price" value={data.entryPrice} />
        <Field label="Available unit range" value={data.availableUnitRange} />
        <Field label="Price verification status" value={data.priceVerificationStatus} />
        <Field label="Rental evidence" value={data.rentalEvidence} />
        <Field label="Investment evidence" value={data.investmentEvidence} />
        <Field label="Construction context" value={data.constructionContext} />
        <Field label="Developer context" value={data.developerContext} />
        <Field label="Liquidity evidence" value={data.liquidityEvidence} />
      </dl>

      <div className="mt-5 border-t border-[#EAE6DE] pt-4">
        <dt className={`text-xs uppercase tracking-wide ${muted}`}>Investment score</dt>
        <dd className={`mt-1 text-sm ${muted}`}>{data.investmentScore}</dd>
      </div>
      ...
      >
        <p className={`text-xs uppercase tracking-wide ${muted}`}>Investment readiness verdict</p>
