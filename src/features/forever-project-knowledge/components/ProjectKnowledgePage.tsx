/**
 * Project Knowledge inspection page (RC5.1).
 *
 * Purely presentational: renders the serialisable
 * {@link ProjectKnowledgeInspection} view-model produced by the engine for
 * any stated project definition. Shows every stage of the RC4.4→RC4.9 chain
 * with full traceability, and shows disputed, withheld, and missing
 * information exactly as the foundations judged it.
 *
 * Project-specific wording comes from the definition's stated copy; every
 * copy field has an honest generic default.
 */

import type { ReactNode } from "react";

import type { ProjectKnowledgeInspection, ProjectKnowledgeReadinessRow } from "../inspection";
...
function ReadinessRows({ rows }: { rows: ProjectKnowledgeReadinessRow[] }) {
  return (
    <>
      {rows.map((row) => (
        <tr key={row.id}>
...
export function ProjectKnowledgePage({ inspection }: { inspection: ProjectKnowledgeInspection }) {
  const copy = inspection.copy ?? {};
  const kicker = copy.kicker ?? "Internal inspection — Forever project knowledge";
  const intro =
    copy.intro ??
    `Committed ${inspection.projectName} source data run end-to-end through the Forever foundations: Project Sources (RC4.4) → Extraction Facts (RC4.5) → Cross-Source Validation (RC4.7) → Canonical Record (RC4.6) → Knowledge Graph (RC4.8) → Readiness (RC4.9). Every value below traces back to a committed source artifact; missing and disputed information is shown, not filled in.`;
  const sourcesNote =
    copy.sourcesNote ??
    "The committed source artifacts this project's facts are stated from. A source that does not exist in the committed package is not registered.";
  const missingNote =
    copy.missingNote ??
    "Fields the project's committed sources genuinely do not state. Each is declared to RC4.7 and reported as an explicit missing_information finding — never given a placeholder value.";
  const readinessNote = [
    `Judged against the caller-stated profile "${inspection.readiness.profileName}".`,
    copy.readinessNote,
  ]
    .filter(Boolean)
    .join(" ");
  const footer =
    copy.footer ??
    'This page is a deterministic inspection of committed repository data. It performs no network calls, reads no database, and fabricates no values: facts absent from the sources appear under "Missing information", and conflicting statements appear under "Disputed information" unresolved.';

  return (
    <div className="bg-[#F3EFE7] px-4 py-10 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <p className="text-xs font-medium tracking-widest text-stone-500 uppercase">{kicker}</p>
        <h1 className="mt-2 font-serif text-4xl text-[#17150F]">
          {inspection.projectName} — Project Knowledge
        </h1>
        <p className="mt-2 max-w-3xl text-sm text-stone-600">{intro}</p>
        ...
        <Section title="Registered sources (RC4.4)" note={sourcesNote}>
          <Table
            headers={[
              "Source",
              "Type",
        ...
        <Section title="Missing information" note={missingNote}>
          {inspection.missing.length === 0 ? (
            <p className="text-sm text-stone-600">No declared gaps.</p>
          ) : (
        ...
        <Section
          ...
          note="Every fact stated by this project's definition, verbatim from committed data, with its source, locator, confidence, and the admissibility RC4.7 assigned it."
        >
        ...
        <Section title="Readiness (RC4.9)" note={readinessNote}>
        ...
        <p className="mt-10 max-w-3xl text-xs text-stone-500">{footer}</p>
      </div>
    </div>
  );
}
