...
function Invoke-ValidationProfile {
  ...
  $results=@();foreach($validation in @($Profile)){
    ...
    if($validation.name -eq 'changed-file-lint'){$files=@($ChangedFiles|Where-Object{$_ -match '\.(js|jsx|ts|tsx|mjs|cjs)$'});if(-not $files.Count){$results+=[pscustomobject]@{Name=$validation.name;Status='passed';Detail="no changed lint-supported files; repository baseline: $LintBaseline";Native=$null};continue};$arguments+=@('--')+$files}
...
function Invoke-ForeverOperator {
  ...
  $inbox=Resolve-ContainedPath $config.patchInbox $configDir 'Patch inbox';$patch=Resolve-ContainedPath $task.patchPath $configDir 'Patch path';if(-not $patch.StartsWith(($inbox.TrimEnd('\','/')+[IO.Path]::DirectorySeparatorChar),[StringComparison]::OrdinalIgnoreCase)){throw "Patch path must stay inside configured inbox: $inbox"};Assert-NoReparseTraversal $patch $configDir 'Patch path'
